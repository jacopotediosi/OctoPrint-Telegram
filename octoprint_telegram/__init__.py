...
class TelegramListener(threading.Thread):
    ...
    def handle_document_message(self, message):
        try:
            ...
            if not self.main.is_command_allowed(chat_id, from_id, "/upload"):
                ...
                self.main.send_msg(
                    f"{Emoji.get('warning')} You are not authorized to upload files",
                    chatID=chat_id,
                )
                return
            ...
            if not octoprint.filemanager.valid_file_type(
                ...
                if uploaded_file_filename.lower().endswith(".zip"):
                    ...
                    self.main.send_msg(
                        f"{Emoji.get('warning')} Sorry, I only accept files with .gcode, .gco or .g or .zip extension",
                        chatID=chat_id,
                    )
                    return
            ...
            self.main.send_msg(
                f"{Emoji.get('save')} Saving file {uploaded_file_filename}...",
                chatID=chat_id,
            )
            ...
            if added_files_relative_paths:
                response_message = f"{Emoji.get('upload')} I've successfully saved the file(s) you sent me as {', '.join(added_files_relative_paths)}"
            else:
                response_message = f"{Emoji.get('warning')} No files were added. Did you upload an empty zip?"
            ...
            self.main.send_msg(
                (
                    f"{Emoji.get('warning')} Something went wrong during processing of your file.\n"
                    f"{Emoji.get('mistake')} Sorry. More details are in octoprint.log."
                ),
                chatID=chat_id,
            )
    ...
    def handle_text_message(self, message, chat_id, from_id):
        ...
        if command not in self.main.tcmd.commandDict:
            ...
            if not self.main._settings.get(["no_mistake"]):
                self.main.send_msg(
                    f"I do not understand you! {Emoji.get('mistake')}", chatID=chat_id
                )
            raise ExitThisLoopException()
        ...
        if self.main.is_command_allowed(chat_id, from_id, command):
            ...
            self.main.send_msg(
                f"You are not allowed to do this! {Emoji.get('notallowed')}",
                chatID=chat_id,
            )
    ...
    def parse_user_data(self, message):
        ...
        if not data["private"]:
            ...
            if chat_id in self.main.chats:
                if (
                    ...
                    self.main.send_msg(
                        f"I don't know you! Certainly you are a nice Person {Emoji.get('heart')}",
                        chatID=chat_id,
                    )
                    raise ExitThisLoopException()
        ...
        if chat_id not in self.main.chats:
            ...
            self.main.send_msg(
                f"{Emoji.get('info')} Now I know you. Before you can do anything, go to OctoPrint Settings and edit some rights.",
                chatID=chat_id,
            )
...
class Utils:
    ...
    def send_telegram_request(self, url, method, **kwargs):
        ...
        request_kwargs = {**kwargs, **default_kwargs}
